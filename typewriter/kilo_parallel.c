...
void editorUpdateSyntax (erow *row){
    ...
    while (i < row->rsize){
        ...
        //this is the keyword
        if (prev_sep){            //check if you have a separator ; if yes
            int j;
...
char *editorPrompt (char *prompt , void (*callback)(char * , int) ){
    ...
    while (1){
        ...
        if (c == DEL_KEY || c == CTRL_KEY('h') || c == BACKSPACE){
            ...
            if (callback) callback(buf , c);                //passes the key and the buffer. Functions use the key and determine what to do
            free(buf);
            return NULL;
        }
        else if (c == '\r'){
